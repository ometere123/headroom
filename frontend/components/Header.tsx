...
export function Header() {
  ...
  if (path === "/") return <header className="landing-head"><Link className="brand" href="/"><img src="/headroom-mark.svg" alt=""/><b>HEADROOM</b></Link><span className="network-tag">GENLAYER STUDIONET Â· 61999</span><Link className="button ghost" href="/control">Enter Control Room</Link></header>;
  ...
  if (!app) return null;
  return <><div className="status-strip"><Link className="compact-brand" href="/control"><img src="/headroom-mark.svg" alt=""/><b>HEADROOM</b></Link><span className="live-led">GENLAYER STUDIONET</span><span>CHAIN 61999</span><a href={CONTRACT_ADDRESS ? `${EXPLORER}/address/${CONTRACT_ADDRESS}` : undefined}>CONTRACT {CONTRACT_ADDRESS ? `${CONTRACT_ADDRESS.slice(0, 7)}â€¦${CONTRACT_ADDRESS.slice(-4)}` : "NOT CONFIGURED"}</a><TimeZoneControl/><WalletButton/></div><aside className="ops-rail"><div className="rail-caption">Operations</div>{items.map(([href,label,num])=><Link className={path===href || (href!=="/control"&&path.startsWith(href+"/")) ? "selected" : ""} href={href} key={href}><small>{num}</small><span>{label}</span></Link>)}<div className="rail-spacer"/><Link href="/protocol"><small>â†—</small><span>Protocol</span></Link><Link href="/"><small>âŒ‚</small><span>Product</span></Link></aside></>;
}
