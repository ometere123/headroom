...
export async function waitFinal(hash:string){const receipt=await readClient().waitForTransactionReceipt({hash:hash as `0x${string}`,waitUntil:"finalized",retries:240,interval:15000} as any);if(receipt.txExecutionResultName!==ExecutionResult.FINISHED_WITH_RETURN)throw new Error(`Transaction finalized without successful execution: ${receipt.statusName} / ${receipt.txExecutionResultName??"NOT_VOTED"}`);return receipt;}
